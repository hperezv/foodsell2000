<?php
class Admin_trabajador extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($tipodoc_id !== false && $search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            */
            ...
            if($search_string){
                if($order){
                    $data['trabajador'] = $this->trabajador_model->get_trabaj($tipodoc_id, $search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['trabajador'] = $this->trabajador_model->get_trabaj($tipodoc_id, $search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['trabajador'] = $this->trabajador_model->get_trabaj($tipodoc_id, '', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
